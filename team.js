...
import {completionNotes} from './work-summary.js';
import {authorized,rpc,db} from './db.js';
import {digest,codeOK,employeeCode} from './actor.js';
export async function teamHandler(req,res){
 ...
 try{
  ...
  if(!owner&&!code)return res.status(401).json({error:'Sign in with your employee access code'});
  let data={...b.data},accessCode;
  if(action==='attention'){
   if(!owner)return res.status(403).json({error:'Owner access required'});
   return res.status(200).json({appointments:await db('fieldops_appointments?status=in.(review,needs_help)&select=id,starts_at,status&order=starts_at.asc&limit=100')});
  }
  if(action==='status'&&data.status==='review')data.text=completionNotes(data.completion);
