...
test('Square: secure checkout, retries, signed webhooks, refund accounting, stale balance and pending locks',async()=>{
 ...
 try{
 const savedKey=process.env.SQUARE_WEBHOOK_SIGNATURE_KEY;process.env.SQUARE_WEBHOOK_SIGNATURE_KEY="wrong-key";
 assert.equal((await call("/api/documents",{action:"square-status"})).status,500);
 process.env.SQUARE_WEBHOOK_SIGNATURE_KEY=savedKey;assert.equal((await call("/api/documents",{action:"square-status"})).data.connected,true);
