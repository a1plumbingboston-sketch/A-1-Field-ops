...
const advice={market_low:null,market_typical:null,market_high:null,recommended_total:225,summary:'Replace the supplied fixture.',risks:[],sources:[],recommended_line_items:[{description:'Fixture replacement labor',quantity:1,unit_price:225,price:225,reason:'One hour, moderate difficulty.'}]};
...
test('an unrelated provider AbortError gets an error response rather than leaving the client hanging',async t=>{
 setup(t);let calls=0;
 global.fetch=async url=>{if(String(url).includes('fieldops_key_status'))return response(true);calls++;throw new DOMException('Upstream connection ended','AbortError');};
