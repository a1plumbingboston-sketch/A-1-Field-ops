...
test('field tools: mobile price book, real photo upload, customer history and email activity',async()=>{
 ...
 await page.getByLabel('Service name',{exact:true}).fill('Faucet installation');await page.getByLabel('Customer item description').fill('Install supplied faucet');await page.locator('#bookPrice').fill('295');await page.getByRole('button',{name:'Save service',exact:true}).click();await page.getByRole('button',{name:'Add to quote',exact:true}).click();
 await page.locator('#closeFieldTools').click();assert.equal(await page.locator('#estimateItems .estimate-item').count(),2);assert.equal(await page.locator('#estimateItems .item-unit').last().inputValue(),'295.00');
