...
test('selecting difficulty updates displayed rate, restoration sync and reset without rewriting quote items', () => {
  ...
  const elements = {
    ...
    estLaborRate: {value: '165', min: '', max: '', dataset: {}},
  ...
  elements.estLaborRate.value = '215'; ctx.window.A1EstimateLabor.sync(); assert.equal(elements.estLaborRate.value, '215');
  elements.estLaborRate.value = '200'; elements.estLaborRate.dataset.autoRate = '200'; elements.estDescriptionQuestion8.value = ''; resetHandlers.reset(); assert.equal(elements.estLaborRate.value, '200');
  ...
  assert.equal(elements.estLaborRate.value, '250');
});
