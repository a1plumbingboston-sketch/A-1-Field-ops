...
export async function sendMail({to,subject,html,pdf,filename}){
 validateDelivery(to);
 let r;
 try{r=await fetch('https://api.resend.com/emails',{method:'POST',headers:{Authorization:`Bearer ${process.env.RESEND_API_KEY}`,'Content-Type':'application/json'},body:JSON.stringify({from:process.env.FIELDOPS_FROM_EMAIL,to:[to.trim()],reply_to:process.env.FIELDOPS_REPLY_TO||'a1plumbingboston@gmail.com',subject,html,...(pdf?{attachments:[{filename,content_type:'application/pdf',content:pdf.toString('base64')}]}:{})})});}
