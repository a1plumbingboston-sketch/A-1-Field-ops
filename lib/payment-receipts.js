...
async function mayText(customer){
 ...
 const messages=await db(`fieldops_text_messages?from_number=eq.${encodeURIComponent(phone)}&order=received_at.desc&limit=100`);
 // A recorded opt-out blocks automatic receipts until explicit opt-in is recorded again.
 return !messages.some(m=>(m.opt_out_type==='STOP'||(!m.opt_out_type&&/^(stop|stopall|unsubscribe|cancel|end|quit|revoke|optout)$/i.test(String(m.body||'').trim())))&&Date.parse(m.received_at)>=Date.parse(pref.created_at));
}
export async function autoPaymentReceipt({invoiceId,event,amount}){
 ...
 try{
  ...
  if(/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)){
   ...
   const record=await start('receipt',invoiceId),url=origin.replace(/\/$/,'')+'/api/documents?feature=payment-receipt&token='+encodeURIComponent(record.token),phone=phoneNumber(customer.phone);
