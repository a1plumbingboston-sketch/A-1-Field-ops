...
export async function buildPdf(snapshot,signature,{signingUrl}={}){
 ...
 const pdf=new PDFDocument({size:'LETTER',margin:28,bufferPages:true,info:{Title:`${c.name||'Customer'} — A-1 ${D.labels[kind]}`,Author:'A-1 Plumbing & Heating Co.'}}),chunks=[];
 ...
 function header(continued=false){pdf.image(snapshot.logo?Buffer.from(snapshot.logo.split(',')[1],'base64'):logo,left,24,{fit:[100,55]});txt('A-1 Plumbing & Heating Co.',138,28,275,12,true);txt('339-224-4517 | a1plumbingboston@gmail.com',138,44,270,8);txt('Plumbing & HVAC | 24/7 Emergency Service',138,56,270,7);txt('BUILT ON TRUST.',138,69,260,7,true);txt(D.labels[kind],415,28,169,kind==='completion'?12:16,true);txt(`#${m.number||''}${continued?' / continued':''}`,415,51,165,9);txt(d.created_at?new Date(d.created_at).toLocaleDateString('en-US'):'',415,65,165,8);rule(85,'#c9202f');y=96;}
 ...
 const right=[location,kind==='receipt'?`Payment record for invoice #${m.number}`:kind==='estimate'?`Valid through: ${d.valid_until||'30 days from issue'}`:`Payment terms: ${d.payment_terms|| (d.due_at?new Date(d.due_at).toLocaleDateString('en-US'):'Due upon receipt, subject to applicable law')}`].filter(Boolean).join('\n');
 y=Math.max(txt(customer,left,y,264,9),txt(right,314,y,270,9))+9;
 if(kind==='receipt'){
  const credits=Number(d.credit_amount||0),balance=Math.round((total-credits-paid)*100)/100;
  const records=m.payments.filter(p=>['paid','succeeded','completed'].includes(p.status));
  ensure(112);pdf.roundedRect(left,y,width,96,10).fill('#f5f0e7');
  txt('PAYMENT RECEIPT',left+16,y+14,310,9,true);
  txt(D.money(paid),left+16,y+34,310,26,true);
  txt('Net payments recorded',left+16,y+68,310,9);
  txt(balance>0?'PARTIAL PAYMENT':balance<0?'CREDIT BALANCE':'NO BALANCE DUE',360,y+20,200,10,true);
  txt('Thank you for choosing A-1.',360,y+43,200,9);y+=112;
  for(const [label,value] of [['Invoice total',total],['Approved credits',credits],['Net payments received',paid],[balance<0?'Credit balance':'Remaining balance',Math.abs(balance)]]){txt(label,left,y,350,10);txt(D.money(value),440,y,138,10,true);y+=23;}rule(y);y+=20;
  txt('PAYMENT HISTORY',left,y,width,9,true);y+=20;
  if(!records.length){y=txt('No completed payments are recorded. This document does not confirm payment.',left,y,width,10)+14;}
  for(const p of records){const date=new Date(p.created_at);const dateText=Number.isFinite(date.getTime())?date.toLocaleDateString('en-US',{timeZone:'America/New_York'}):'Date not recorded';const method=String(p.method||'Method not recorded').replace(/_/g,' ');const row=`${dateText}  |  ${method}${Number(p.amount)<0?'  |  Refund / reversal':''}`;const h=Math.max(20,height(row,390,10)+8);ensure(h+8);txt(row,left,y,390,10);txt(D.money(p.amount),440,y,138,10,true);y+=h;rule(y);y+=8;}
  ensure(90);y+=10;y=txt('This receipt records completed payments and any recorded refunds against the invoice. Pending or failed payments are excluded. It does not change your original invoice, signed agreement, or warranty terms.',left,y,width,9)+12;
  txt('Questions about a payment? Call 339-224-4517.',left,y,width,10,true);
  const pages=pdf.bufferedPageRange();for(let i=0;i<pages.count;i++){pdf.switchToPage(i);pdf.font('Helvetica').fontSize(7).fillColor('#555').text(`A-1 Plumbing & Heating Co. | Payment receipt | ${i+1} / ${pages.count}`,left,750,{width,lineBreak:false});}
