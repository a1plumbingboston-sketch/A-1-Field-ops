...
export async function buildPdf(snapshot,signature,{signingUrl}={}){
 ...
 const right=[location,kind==='estimate'?`Valid through: ${d.valid_until||'30 days from issue'}`:`Payment terms: ${d.payment_terms|| (d.due_at?new Date(d.due_at).toLocaleDateString('en-US'):'Due upon receipt, subject to applicable law')}`].filter(Boolean).join('\n');
