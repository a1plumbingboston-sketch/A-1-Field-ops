...
export async function buildPdf(snapshot,signature){
 ...
 const ah=height(m.authorization,width,8)+ (signature?107:60);ensure(ah);rule(y);y+=8;txt(kind==='receipt'?'PAYMENT ACKNOWLEDGMENT':'CUSTOMER AUTHORIZATION / E-SIGNATURE',left,y,width,8,true);y+=13;y=txt(m.authorization,left,y,width,8)+7;
 if(signature){pdf.image(Buffer.from(signature.signature_data.split(',')[1],'base64'),left,y,{fit:[180,60]});y+=63;y=txt(`${signature.customer_name} | Signed ${new Date(signature.signed_at).toISOString()}`,left,y,width,8)+5;if(signature.exceptions)y=txt('Completion exceptions: '+signature.exceptions,left,y,width,8)+5;}else if(kind!=='receipt'){y=txt('Customer signature ______________________________    Date ______________',left,y+14,width,9)+8;}
