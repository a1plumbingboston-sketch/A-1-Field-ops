import {safePaymentReceipt} from './payment-receipts.js';
...
export async function checkSquare(){
 ...
 const {subscription:details}=await square('/v2/webhooks/subscriptions/'+encodeURIComponent(subscription.id));
 const saved=Buffer.from(process.env.SQUARE_WEBHOOK_SIGNATURE_KEY),actual=Buffer.from(details?.signature_key||'');
 if(!actual.length||saved.length!==actual.length||!timingSafeEqual(saved,actual))throw new Error('Square webhook signature key does not match this subscription. Update the saved key and redeploy.');
...
async function record(payment,sendReceipt=true){
 ...
 if(payment.status==='COMPLETED'){await rpc('fieldops_square_record',{p_checkout:c.id,p_payment:payment.id,p_event:'payment:'+payment.id,p_amount:Number(c.amount)});if(sendReceipt)await safePaymentReceipt({invoiceId:c.invoice_id,event:'square:'+payment.id,amount:Number(c.amount)});}
...
async function webhook(req,res){
 ...
 const {payment}=await square('/v2/payments/'+encodeURIComponent(refund.payment_id));if(await record(payment,false)){
