...
export async function checkSquare(){
 ...
 if(!subscription||subscription.signature_key!==process.env.SQUARE_WEBHOOK_SIGNATURE_KEY||!['payment.created','payment.updated','refund.created','refund.updated'].every(x=>subscription.event_types.includes(x)))throw new Error('Square webhook settings do not match. Check the signature key and payment/refund events.');
...
async function webhook(req,res){
 ...
 const id=event.data?.object?.payment?.id;if(id){const {payment}=await square('/v2/payments/'+encodeURIComponent(id));await record(payment);}
