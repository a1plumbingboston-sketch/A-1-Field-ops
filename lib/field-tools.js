...
export async function fieldTools(req,res){
 ...
 if(String(b.op||'').startsWith('finance-'))return res.json(await financeTool(b));
 switch(b.op){
 ...
 case 'deliveries': {
  ...
  if(b.refresh)for(const row of rows.slice(0,5)){
   ...
   try{if(/^SM[0-9a-f]{32}$/i.test(row.provider_id)){const account=process.env.TWILIO_ACCOUNT_SID;const response=await fetch(`https://api.twilio.com/2010-04-01/Accounts/${account}/Messages/${row.provider_id}.json`,{headers:{Authorization:'Basic '+Buffer.from(account+':'+process.env.TWILIO_AUTH_TOKEN).toString('base64')},signal:AbortSignal.timeout(8000)});const message=await response.json();if(!response.ok)throw Error('Text delivery status unavailable');if(['queued','sending','sent','delivered','undelivered','failed','canceled'].includes(message.status)){row.status=message.status;row.checked_at=new Date().toISOString();await db('fieldops_deliveries?id=eq.'+row.id,{method:'PATCH',body:{status:row.status,checked_at:row.checked_at}});}continue;}const r=await fetch('https://api.resend.com/emails/'+encodeURIComponent(row.provider_id),{headers:{Authorization:'Bearer '+process.env.RESEND_API_KEY},signal:AbortSignal.timeout(8000)});const j=await r.json();if(!r.ok)throw new Error('Email provider status unavailable');
