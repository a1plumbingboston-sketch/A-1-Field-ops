...
const form=$('#taskEstimatorForm');let result=null,inputSnapshot='',inputContext='',revision=0,activeRequest=null;
const contextId=()=>String(window.A1RemodelBridge?.contextId?.()??'');
// Read named controls directly so saving a disabled form preserves its intake.
const inputs=()=>Object.fromEntries([...form.elements].filter(el=>el.name).map(el=>[el.name,el.value]));
...
function resetResult(){revision++;activeRequest?.abort();activeRequest=null;result=null;inputSnapshot='';inputContext='';$('#taskResult').innerHTML='';$('#buildTaskEstimate').disabled=false;}
...
function restore(value){
 resetResult();form.reset();$('#taskStatus').textContent='';
 if(!value||typeof value!=='object'||Array.isArray(value))return;
 for(const el of form.elements){if(!el.name||!Object.hasOwn(value,el.name))continue;const v=value[el.name];if(v===null)el.value='';else if(['string','number'].includes(typeof v))el.value=String(v);}
}
function reset(){restore(null);try{localStorage.removeItem(STORAGE_KEY);}catch{}}
function openEstimator(){box.hidden=false;box.scrollIntoView({behavior:'smooth',block:'start'});}
window.A1TaskEstimator=Object.freeze({capture:inputs,restore,reset,open:openEstimator});
try{restore(JSON.parse(localStorage.getItem(STORAGE_KEY)||'null'));}catch{}
open.onclick=openEstimator;
$('#closeTaskEstimator').onclick=()=>{box.hidden=true;open.focus();};
$('#clearTaskEstimator').onclick=()=>{if(!confirm('Clear this walkthrough? Saved quotes stay unchanged.'))return;reset();document.dispatchEvent(new Event('fieldops:construction-change'));};
function intakeChanged(){resetResult();save();document.dispatchEvent(new Event('fieldops:construction-change'));}
form.addEventListener('input',intakeChanged);form.addEventListener('change',intakeChanged);
...
form.onsubmit=async e=>{
 ...
 if(description.length>20000){$('#taskStatus').textContent='Shorten the walkthrough, location, site conditions or exclusions to a combined 20,000 characters before allocating. Nothing has been removed or sent.';return;}
 if(!Number.isFinite(total)||total<=0){$('#taskStatus').textContent='Enter the total project price you want itemized.';return;}
 inputSnapshot=JSON.stringify(inputs());inputContext=contextId();const generation=revision,request=new AbortController();activeRequest=request;btn.disabled=true;$('#taskStatus').textContent='Organizing the scope into fixed-price line items…';
 try{
  const r=await fetch('/api/invoice-assist',{method:'POST',headers:{'Content-Type':'application/json','x-fieldops-key':window.A1RemodelBridge.key()},body:JSON.stringify({mode:'allocate',document_type:'estimate',profile:'construction',title:x.project,description,total}),signal:request.signal});
  const j=await r.json();if(generation!==revision)return;if(inputContext!==contextId())throw Error('The estimate changed while AI was working. Allocate again for the current estimate.');if(!r.ok)throw Error(j.error||'Could not allocate this project price.');
 ...
 }catch(err){if(generation===revision)$('#taskStatus').textContent=err.name==='AbortError'?'Price allocation canceled. Your walkthrough is unchanged.':err.message;}finally{if(generation===revision){activeRequest=null;btn.disabled=false;}}
};
...
function render(x,j){
 ...
 $('#applyTaskEstimate').onclick=async()=>{
  if(inputContext!==contextId()){resetResult();$('#taskStatus').textContent='The estimate changed. Allocate again for the current estimate.';return;}if(inputSnapshot!==JSON.stringify(inputs())||!$('#reviewTaskEstimate').checked||!result)return;
  const btn=$('#applyTaskEstimate');btn.disabled=true;
  try{const applied=await window.A1RemodelBridge.apply({title:x.project,description:scope,items:result.items,profile:'construction',intake:inputs()});if(applied){box.hidden=true;$('#estimateForm').scrollIntoView({behavior:'smooth',block:'start'});}}
