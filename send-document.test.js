...
test('a second send is rejected while the first is still in flight for the same document, but a later resend after it resolves succeeds',async()=>{
 const app=await startServer();const original=global.fetch;
 try{
  const call=async(body)=>{const r=await fetch(app.origin+'/api/send-document',{method:'POST',headers:{'Content-Type':'application/json','x-fieldops-key':'test-key'},body:JSON.stringify(body)});return {status:r.status,...await r.json()};};
  const inv=(await app.q("insert into invoices(customer_id,title,total,subtotal) values($1,'Race test',100,100) returning id",[app.customer]))[0].id;
  await app.q("update customers set email='race@example.test' where id=$1",[app.customer]);
  let releaseFirst;const gate=new Promise(r=>releaseFirst=r);let resendCalls=0;
  global.fetch=async(url,opts)=>{
   if(String(url)==='https://api.resend.com/emails'){resendCalls++;if(resendCalls===1)await gate;app.mails.push(JSON.parse(opts.body));return Response.json({id:'race-mail-'+resendCalls});}
   return original(url,opts);
  };
  const body={kind:'invoice',id:inv};
  const first=call(body);
  await new Promise(r=>setTimeout(r,20));
  const second=await call(body);assert.equal(second.status,409);assert.match(second.error,/already in progress/);
  releaseFirst();
  assert.equal((await first).sent,true);
  const third=await call(body);assert.equal(third.sent,true);
