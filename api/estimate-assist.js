...
The user-entered job.labor_rate is authoritative for labor in hourly estimates and internal fixed-task calculations. Job difficulty is context for scope, risk and labor-hour reasoning only; it must never change or multiply the hourly rate. Never substitute a market rate or add a difficulty multiplier to this hourly rate. Site conditions may change the actual time required, but do not also add a separate difficulty fee for the same condition. These are customer billing rates, not technician wages or actual labor costs.
